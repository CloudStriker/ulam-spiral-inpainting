...
# Mask generator
def generate_mask(batch_size, channels, height, width, mask_ratio=0.35):
    return (
...
class CombinedLoss(nn.Module):
    ...
    def forward(self, preds, targets):
        targets = (targets > 0.5).float()
        return self.alpha * self.soft_mca(preds, targets) + self.beta * self.bce(
            preds, targets
        )
...
if __name__ == "__main__":
    ...
    # Change this to 25,000,000; 50,000,000; 100,000,000; 200,000,000; 300,000,000; 400,000,000; 500,000,000
    ...
    wandb.init(
        project="ulam-spiral-painting-train", config=config, name=f"train_{model_name}"
    )
