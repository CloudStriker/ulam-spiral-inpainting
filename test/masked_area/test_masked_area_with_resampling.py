...
def calculate_metrics_from_counts_list(counts_list):
    ...
    class_0_percent = (
        ((total_tn + total_fp) / total_pixels) * 100 if total_pixels > 0 else 0
    )
    class_1_percent = (
        ((total_tp + total_fn) / total_pixels) * 100 if total_pixels > 0 else 0
    )
    ...
    return {
        ...
        "Class 0 Percent": class_0_percent,
        "Class 1 Percent": class_1_percent,
    }


def calculate_detailed_metrics_per_image(pred_binary, target, mask):
...
def save_comparison_image(images, metrics, save_path):
    ...
    canvas = Image.new(
        "RGB", (panels_total_width + sidebar_width, total_height), (0, 0, 0)
    )
    ...
    for i, img in enumerate(images):
        ...
        draw.text(
            (i * panel_width + (panel_width - text_w) // 2, 5),
            titles[i],
            font=font_bold,
            fill=(255, 255, 255),
        )
    ...
    y_pos += 25
    draw.line(
        [(x_pos - 5, y_pos - 5), (sidebar_x1 - 10, y_pos - 5)],
        fill=(80, 80, 80),
        width=1,
    )
    ...
    draw.text(
        (x_pos, y_pos),
        f"  Black (0): {metrics['class_0']['percent']:.1f}%",
        font=font,
        fill=(200, 200, 200),
    )
    y_pos += 18
    draw.text(
        (x_pos, y_pos),
        f"  White (1): {metrics['class_1']['percent']:.1f}%",
        font=font,
        fill=(200, 200, 200),
    )
    y_pos += 25
    ...
    draw.text(
        (x_pos, y_pos),
        f"Balanced Accuracy (mCAcc): {metrics['balanced_acc']:.2f}%",
        font=font_bold,
        fill=(255, 255, 255),
    )
    y_pos += 25
    ...
    draw.text(
        (x_pos, y_pos),
        f"  Precision: {metrics['class_0']['precision']:.2f}%",
        font=font,
        fill=(200, 200, 200),
    )
    y_pos += 18
    draw.text(
        (x_pos, y_pos),
        f"  Recall: {metrics['class_0']['recall']:.2f}%",
        font=font,
        fill=(200, 200, 200),
    )
    y_pos += 18
    draw.text(
        (x_pos, y_pos),
        f"  F1-Score: {metrics['class_0']['f1']:.2f}%",
        font=font,
        fill=(200, 200, 200),
    )
    y_pos += 25
    ...
    draw.text(
        (x_pos, y_pos),
        f"  Precision: {metrics['class_1']['precision']:.2f}%",
        font=font,
        fill=(200, 200, 200),
    )
    y_pos += 18
    draw.text(
        (x_pos, y_pos),
        f"  Recall: {metrics['class_1']['recall']:.2f}%",
        font=font,
        fill=(200, 200, 200),
    )
    y_pos += 18
    draw.text(
        (x_pos, y_pos),
        f"  F1-Score: {metrics['class_1']['f1']:.2f}%",
        font=font,
        fill=(200, 200, 200),
    )
    y_pos += 25
    ...
    draw.text(
        (x_pos + 18, y_pos), "TN (Correct Black)", font=font, fill=(200, 200, 200)
    )
    y_pos += 18
    draw.rectangle([(x_pos, y_pos), (x_pos + 12, y_pos + 12)], fill=(255, 255, 255))
    draw.text(
        (x_pos + 18, y_pos), "TP (Correct White)", font=font, fill=(200, 200, 200)
    )
    y_pos += 18
...
def generate_mask(batch_size, channels, height, width, mask_ratio=0.2, seed=None):
    ...
    mask = (
        (torch.rand(batch_size, channels, height, width) > (1 - mask_ratio))
        .float()
        .to(DEVICE)
    )
    if seed is not None:
...
def test_model(model, dataloader, model_name="", mask_ratio=0.2, R=1000, ci=95):
    ...
    with torch.no_grad():
        for i, (data, target, original_names) in enumerate(
            tqdm(dataloader, desc="Prediksi")
        ):
            ...
            for j in range(data.size(0)):
                ...
                stats = calculate_detailed_metrics_per_image(
                    output_binary[j], target[j], mask[j]
                )
    ...
    micro_f1_res = next(
        (item for item in results_summary if item["Metric"] == "Overall Accuracy"), None
    )
    macro_f1_res = next(
        (item for item in results_summary if item["Metric"] == "Macro F1"), None
    )
    ...
    for res in results_summary:
        if "Class" in res["Metric"]:
            line = (
                f"  {res['Metric']:<20}: {res['Point Estimate']} "
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--model-path", type=str, required=True, help="Path to trained model .pth file"
    )
    parser.add_argument(
        "--test-dir",
        type=str,
        default="prime_patches/test",
    ...
    parser.add_argument(
        ...
        type=str,
        required=True,
        help="Filter for image size (e.g., '25,000,000')",
    )
    parser.add_argument(
        "--batch-size", type=int, default=4, help="Batch size for testing"
    )
    parser.add_argument(
        "--mask-ratio", type=float, default=0.2, help="Ratio of pixels to mask (0-1)"
    )
    parser.add_argument(
        "--R", type=int, default=10000, help="Number of bootstrap replicates"
    )
    parser.add_argument(
        "--ci",
        type=int,
        default=95,
        help="Confidence interval level (e.g., 95 for 95%%)",
    )
    ...
    test_dataset = SimpleImageDataset(
        root_dir=args.test_dir, transform=transform, size_filter=args.size_filter
    )

    if len(test_dataset) == 0:
        print(
            f"❌ ERROR: No test data found in '{args.test_dir}' with filter '{args.size_filter}'."
        )
        print("Make sure path and filter is correct.")
    else:
        print(
            f"✅ Found {len(test_dataset)} test images with filter '{args.size_filter}'."
        )
